...
class Loaded_Blockchain:
    def __init__(self,Number_of_blocks: int, Number_of_elements: int) -> None:
        ...
        self.path = global_path + "Blockchain Database"
    ...
    def Check_bloom_filters_for_element(self,element):
        ...
        for i in range(self.Number_of_blocks):
            ...
            if bytes("0." + str(element)[2:-1],'utf-8') in Bloom_filter:
    ...
    def Retrieve_Element_BloomFIlter(self,element):
        ...
        # lines = csv.reader(Block_element_file)
        lines = Block_element_file.readlines()
        bloom_lines = Bloom_filter_file.readlines()
        Block_element_file.close()
        Bloom_filter_file.close()
        ...
        for i, line in enumerate(lines):
            Bloom_filter = BloomFilter(int(bloom_lines[i]))
            # line = list(line.split(","))
            # line[-1] = line[-1][:-1]
    ...
    def Retrieve_Element_BloomFIlter2(self,element):
        ...
        med = time.time()
        for block_number in Block_numbers_of_positive_BF:
            # time.sleep(0.0001)
            if str(element)[2:-1] in lines[block_number]:
                self.Block_numbers_retrieved_BloomFilter2.append(block_number)
...
def Create_Blockchain_Data(Num_Of_Blocks: int,Num_Of_Elements: int) -> None:
    ...
    path = global_path + "Blockchain Database/"
...
def Test_Loaded_Blockchain_Time_performance(list_of_number_of_Blocks:list, list_of_number_of_Elements:list, iteration_times:int,  element:bytes):
    ...
    Final_BruteForce_list = np.zeros((len(list_of_number_of_Elements), len(list_of_number_of_Blocks)))
    Final_BloomFilter_list = np.zeros((len(list_of_number_of_Elements), len(list_of_number_of_Blocks)))
    ...
    for i in range(iteration_times):
        ...
        Final_BruteForce_list = np.add(Final_BruteForce_list, REBruteForce_result_array)
        Final_BloomFilter_list = np.add(Final_BloomFilter_list, REBloomFilter_result_array)

        REBruteForce_result_array = []
        REBloomFilter_result_array = []

        Final_Brute_time = Final_Brute_time + med - start
        Final_Bloom_time = Final_Bloom_time + end - med

        print("Iteration: ", i+1, "/", iteration_times)
    ...
    ax1.set_yticklabels(labels=[str(x) for x in list_of_number_of_Elements])
    ax1.set_xticks(np.arange(len(data1[0])))
    ax1.set_xticklabels(labels=[str(x) for x in list_of_number_of_Blocks])

    ax1.set_ylabel("Number of elements")
    ax1.set_xlabel("Number of Blocks")

    ax2.set_yticks(np.arange(len(data2)))
    ax2.set_yticklabels(labels=[str(x) for x in list_of_number_of_Elements])
    ax2.set_xticks(np.arange(len(data2[0])))
    ax2.set_xticklabels(labels=[str(x) for x in list_of_number_of_Blocks])
    ...
    print("Total execution time of fake data retrieval is: ",  time.strftime("%H:%M:%S", time.gmtime(end - start)))
    plt.show()

    # return REBruteForce_result_array, REBloomFilter_result_array, Final_Brute_time, Final_Bloom_time
